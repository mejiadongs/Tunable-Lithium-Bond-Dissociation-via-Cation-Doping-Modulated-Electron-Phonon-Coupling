...
for folder in folders:
    ...
    if metal_elements:
        ...
        try:
            ...
            with open(os.path.join(cwd, 'd_band_center.txt'), 'a') as f:  # Use the full path to the file
                f.write(f"{folder}\n")
            
            # Extract data from BAND_CENTER
            with open(os.path.join(folder, 'BAND_CENTER'), 'r') as f:
                content = f.read()
                spin_data = re.findall(r'(Spin-Channel|Spin-UP|Spin-DW)\s+\S+\s+\S+\s+(\S+)', content)
            ...
            os.rename(os.path.join(folder, 'BAND_CENTER'), os.path.join(folder, 'BAND_CENTER_d'))  # add the directory to the file path

            print(f"{folder} 处理完成")
        except Exception as e:
            print(f"Error processing {folder}: {str(e)}")
            with open(os.path.join(cwd, 'error_d_band_center.txt'), 'a') as f:
