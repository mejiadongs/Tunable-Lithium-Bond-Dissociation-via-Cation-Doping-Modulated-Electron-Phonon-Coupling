...
# -*- coding: utf-8 -*-
"""
RIXS phonon analysis pipeline (v3.1)
------------------------------------
Key changes from v3.0 → v3.1
- (New) Asymmetric least-squares (asLS) baseline option (robust elastic-tail / slow background removal)
- (New, automatic) Heatmap masks energy-loss bins below the elastic-exclusion threshold to avoid bogus stripes
- (New) Optional flux normalization per spectrum (divide by monitor/flux column if present)
- (Polish) Safer log-scale handling and NaN-aware smoothing/regularization
- (Compat) CLI flags added without breaking previous defaults

Example
  python rixs_phonon_analyzer_v3_1.py \
    --scan SJD1_18030-AI.txt \
    --andor-dir ./Andor \
    --calib-a 0.12666 --calib-b 449.99336 \
    --window-meV 500 --phonon-band 80,160 \
    --map-bin-meV 25 --map-smooth 1.0 --map-norm rowmax \
    --excl-policy fixed --excl-meV 60 \
    --plot --out-prefix rixs \
    --baseline-asls --asls-lam 5e6 --asls-p 1e-3 --flux-norm

Notes
- asLS baseline is applied after optional Bose correction / deconvolution and before integrations.
- Heatmap columns with ΔE below the effective exclusion width are set to NaN (transparent), so they
  do not bias color scaling when using normalization.
"""
...
def read_scan_file(scan_path: str) -> pd.DataFrame:
    ...
    Adds convenience columns: FileKey (xxxxx) and OneDName (=Filename + '-1D').
    """
...
def asls_baseline(y: np.ndarray, lam: float = 5e6, p: float = 1e-3, niter: int = 10) -> np.ndarray:
    """Eilers & Boelens (2005) asLS baseline using sparse Whittaker smoothing.
    Parameters
    ----------
    lam : smoothing strength (larger → smoother baseline)
    p   : asymmetry (small p → baseline stays under peaks)
    ...
    # Second-difference operator (L-2 x L)
...
class RIXSDataProcessorFixed:
    ...
    def plot_corrected_overview(self, out_png: str = 'rixs_fixed_overview.png'):
        ...
        for k in self.ordered_keys:
            ...
            y_norm = y / (np.nanmax(y) if (np.isfinite(np.nanmax(y)) and np.nanmax(y) > 0) else 1.0)
...
class RIXSPhononAnalyzer:
    ...
    def _pick_flux_col(self, df: pd.DataFrame) -> Optional[str]:
        candidates = [
            'Izero', 'I0', 'Izero (a.u.)', 'Total Flux', 'Flux', 'Total Intensity',
            'Izero/Flux', 'Izero/Total Flux', 'Monitor'
        ]
        for c in candidates:
            if c in df.columns:
                vals = pd.to_numeric(df[c], errors='coerce')
                if np.isfinite(vals).sum() >= max(3, int(0.2 * len(vals))):
                    return c
        return None
    ...
    def analyze_phonon_region(self,
                              ...
                              map_log: bool = False,
                              map_regularize: bool = True,
                              baseline_asls: bool = False,
                              asls_lam: float = 5e6,
                              asls_p: float = 1e-3,
                              asls_iters: int = 10,
                              flux_norm: bool = False) -> Dict[str, np.ndarray]:
        ...
        # pick flux column if available
        flux_col = None
        if flux_norm and self.processor.scan_data is not None:
            flux_col = self._pick_flux_col(self.processor.scan_data)

        excl_values = []  # keep track of used exclusion widths for masking later
        ...
        for i, k in enumerate(keys):
            ...
            # (v3.1) optional baseline
            if baseline_asls:
                bg = asls_baseline(If_proc, lam=float(asls_lam), p=float(asls_p), niter=int(asls_iters))
                If_proc = If_proc - bg
                If_proc[If_proc < 0] = 0

            # optional flux normalization
            if flux_col and self.processor.scan_data is not None and 'FileKey' in self.processor.scan_data.columns:
                vals = self.processor.scan_data.loc[self.processor.scan_data['FileKey'] == k, flux_col].values
                if len(vals):
                    fv = float(pd.to_numeric(vals[0], errors='coerce'))
        ...
        if map_regularize and len(inc_arr) > 2 and heat.shape[0] > 2:
            ...
            heat = heat_reg
            inc_arr = y_reg
        # (v3.1) mask bins below median exclusion width across spectra
        excl_med = float(np.nanmedian(excl_values)) if len(excl_values) else 0.0
        cut_col = int(np.searchsorted(bins, excl_med, side='right'))
        if heat.size > 0 and cut_col > 0 and cut_col <= heat.shape[1]:
            heat[:, :cut_col] = np.nan
        # 2D Gaussian smoothing (NaN-safe: replace with 0 for filtering, keep mask)
        if map_smooth and heat.size > 4:
            mask = np.isnan(heat)
            tmp = heat.copy()
            tmp[mask] = 0.0
            try:
                tmp = gaussian_filter(tmp, sigma=float(map_smooth))
                mfloat = (~mask).astype(float)
                mfloat = gaussian_filter(mfloat, sigma=float(map_smooth))
                with np.errstate(invalid='ignore'):
                    heat = tmp / np.where(mfloat == 0, np.nan, mfloat)
            except Exception:
                pass
        ...
        if map_log:
            ...
            posvals = heat[heat > 0]
            vmin = float(np.nanpercentile(posvals, 5)) if posvals.size else 1e-6
            vmax = float(np.nanpercentile(heat[np.isfinite(heat)], 99.5)) if np.isfinite(heat).any() else 1.0
            if vmin <= 0:
                vmin = max(1e-9, 0.001 * (vmax if np.isfinite(vmax) else 1.0))
        ...
        # 5) Phonon DOS proxy (from mid spectrum)
        ...
        inc = [float(self.processor.scan_data.loc[self.processor.scan_data['FileKey'] == k, 'BL 8 Energy'].values[0])
               if self.processor.scan_data is not None and 'BL 8 Energy' in self.processor.scan_data.columns else np.nan
               for k in keys]
        pos = self.elastic_positions_eV
        fwhm = self.elastic_fwhm_meV
        # (v3.1+) robust guard against occasional bad fits in elastic position
        pos_plot = pos.copy()
        try:
            if pos_plot is not None and len(pos_plot) > 3 and np.isfinite(pos_plot).any():
                med = float(np.nanmedian(pos_plot))
                mad = float(np.nanmedian(np.abs(pos_plot - med)))
                thr = max(0.25, 6.0 * mad) if (np.isfinite(mad) and mad > 0) else 0.25  # ~0.25 eV or 6*MAD
                bad = np.abs(pos_plot - med) > thr
                if np.any(bad):
                    xp = np.flatnonzero(~bad)
                    if xp.size >= 2:
                        pos_plot[bad] = np.interp(np.flatnonzero(bad), xp, pos_plot[xp])
                    else:
                        pos_plot[bad] = np.nan
        except Exception:
            pos_plot = pos
        ...
        ax6_t = ax6.twinx()
        l1 = ax6.plot(inc, pos_plot, 'bo-', ms=5, label='Elastic peak position (eV)')
        ...
        # keep NaNs from masked low-ΔE columns
...
def main():
    ap = argparse.ArgumentParser(description='RIXS phonon analysis (ΔE axis, robust fitting, exports) [v3.1]')
    ...
    # v3.1 options
    ap.add_argument('--baseline-asls', action='store_true', help='Enable asLS baseline removal before integration')
    ap.add_argument('--asls-lam', type=float, default=5e6, help='asLS smoothing strength (Whittaker λ)')
    ap.add_argument('--asls-p', type=float, default=1e-3, help='asLS asymmetry parameter p (small→baseline under peaks)')
    ap.add_argument('--asls-iters', type=int, default=10, help='asLS iteration count')
    ap.add_argument('--flux-norm', action='store_true', help='Normalize each spectrum by monitor/flux column if present in scan file')
    # misc
    ...
    res = analyzer.analyze_phonon_region(
        ...
        baseline_asls=args.baseline_asls,
        asls_lam=args.asls_lam,
        asls_p=args.asls_p,
        asls_iters=args.asls_iters,
